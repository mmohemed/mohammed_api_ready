...
| 🔍 الجودة | شاشة مراقبة جودة: نسب العيوب لكل أمر + كشف الأوامر الشاذة بالذكاء الاصطناعي |
| 🕐 الحضور والانصراف | تسجيل يدوي من الواجهة أو تلقائي من أجهزة البصمة عبر `POST /integrations/attendance` بمفاتيح API |
| 📊 تقارير Excel | المخزون، المبيعات، القيود، الحضور — بزر واحد من كل شاشة |
| 🔖 باركود وQR | صورة QR لكل منتج للطباعة، وبحث فوري يدعم قارئ الباركود في شاشة المخازن |
| 🔌 مفاتيح التكامل | إنشاء/إيقاف مفاتيح API لربط أجهزة البصمة والأنظمة الخارجية (ترويسة `X-API-Key`) |
| 📱 تطبيق Flutter | هيكل تطبيق مشرفين جاهز للبناء في مجلد `mobile_app/` (والواجهة نفسها متجاوبة مع الجوال) |
